...
#setwd("current_dir/")
tabula_marrow = readRDS("marrow.rds")
spliced = readRDS("spliced.rds")
unspliced = readRDS("unspliced.rds")
...
ggplot(figcelltype, aes(y = reorder(CellType, Transcripts, median), x =
                          Transcripts)) +  geom_density_ridges(scale = 2, fill = "#35A2FF") + scale_y_discrete(position = "right") +
  theme_classic() + theme() + NoLegend() +
  theme(plot.margin = unit(c(0, 0, 0, 0), "cm"),
        axis.text = element_text(color = "black")) + labs(title = "", y = "", x = "log2 Transcripts") #450 x 500
