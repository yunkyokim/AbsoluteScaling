...
granulocyte@assays$RNA@data = as.matrix(log2(granulocyte@assays$RNA@counts +
                                               1))
granulocyte <-
...
FeaturePlot(granulocyte,
            ...
            pt.size = 0.05) + scale_color_viridis_c() +
  theme(
    aspect.ratio = 1,
...
clu_df <- lapply(names(rcl.list), function(i) {
  ...
  do.call(rbind, .)
write.csv(clu_df, "granulocyte_pseudotime_clusters.csv")
...
auto_goplot = function(filepath, color) {
  ...
  print(
    ggplot(head(david, 10), aes(x = logp, y = reorder(Name, logp)),) +
      geom_bar(stat = "identity",  fill = color) + theme_classic() +
      NoLegend() +
      theme(
        plot.margin = margin(
          t = 5,
