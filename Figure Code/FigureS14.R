...
tabula_skin[["unspliced"]] <-
  CreateAssayObject(counts = unspliced_ercc)

tabula_skin@assays$RNA@data = as.matrix(log2(tabula_skin@assays$RNA@data +
                                               1))
tabula_skin <-
  FindVariableFeatures(tabula_skin,
                       selection.method = "vst",
                       nfeatures = 2000)
all.genes <- rownames(tabula_skin)
tabula_skin <- ScaleData(tabula_skin, features = all.genes)
tabula_skin <-
  RunPCA(tabula_skin, features = VariableFeatures(object = tabula_skin))
...
ggplot(figcelltype, aes(y = reorder(CellType, Transcripts, median), x =
                          Transcripts)) +  geom_density_ridges(scale = 2, fill = "#C77CFF") + scale_y_discrete(position = "right") +
